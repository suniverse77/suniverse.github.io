...
각 $p_\theta^{(t)}(\mathbf{x}\_{t-1}\mid\mathbf{x}\_t)$는 $q\_\sigma(\mathbf{x}\_t \mid \mathbf{x}\_{t-1}, \mathbf{x}\_0)$에 대한 지식을 활용한다. 
...
$$
\mathbf{x}_{t-1} =
\sqrt{\alpha_{t-1}} \left( 
\underbrace{
\frac{
\mathbf{x}_t - \sqrt{1 - \alpha_t} \, \boldsymbol{\epsilon}_\theta^{(t)}(\mathbf{x}_t)
}{
\sqrt{\alpha_t}
}
