...
spatial layer의 출력 $\mathbf{z}__i$와 temporal layer의 출력 $\mathbf{z'}__i$는 $\mathbf{z}__{i+1}=\alpha__\phi^i\mathbf{z}+(1-\alpha_\phi^i)\mathbf{z'}$ 형태로 결합되어 다음 layer로 전달된다. ($\alpha$는 학습 가능한 파라미터임)
