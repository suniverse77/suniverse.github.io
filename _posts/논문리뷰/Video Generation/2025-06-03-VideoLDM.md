...
- 1단계는 1절과 2절에서 설명한 방법으로, 의미적으로 큰 변화가 있는 key frame들을 생성하는 단계이다. 그러나 이 방식은 메모리 제약으로 인해 비교적 낮은 프레임 레이트로만 생성이 가능하다.
- 2단계에서는 key frame 사이를 보간하는 추가적인 모델을 도입한다. 이를 구현하기 위해 2절에서 제안한 masking-conditioning 메커니즘을 그대로 활용한다. 그러나 예측 과제와 달리, 이번에는 보간 대상인 프레임들을 마스킹한다는 점이 다르다. 나머지 메커니즘은 동일하며, 즉 기존의 이미지 생성 모델을 비디오 보간 모델로 확장하는 것이다.
...
우리의 예측 및 보간 모델 학습 방식은, 유사한 마스킹 기법을 사용하는 최근 연구들 [24, 33, 93]에서 영감을 받았으며,
