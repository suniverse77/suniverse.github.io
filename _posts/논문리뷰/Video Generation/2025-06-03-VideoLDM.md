...
- Top: temporal 디코더를 파인튜닝하는 과정을 나타내며, video-aware discriminator를 도입해 비디오의 시간적 일관성을 고려하도록 하였다. (GAN의 discriminator와는 다름)
- Bottom: latent space에서 LDM의 denoising 과정을 나타낸다. (학습한 분포에서 샘플링된 latent code를 디코더에 통과시키면 특정 이미지가 출력) 

### Turning Latent Image into Video Generators

이러한 방식의 핵심적인 장점은 방대한 이미지 데이터셋을 활용하여 공간 계층을 사전 학습할 수 있고, 상대적으로 덜 확보된 비디오 데이터는 시간 계층 학습에만 집중적으로 사용할 수 있다는 점이다.
...
최종 학습 목표는 Our video-aware temporal backbone is then trained using the same noise schedule as the underlying image model,
and, i

$$
\underset{\phi}{\argmin}\mathbb{E}_{\mathbf{x}}\big[\lVert\mathbf{y}-\mathbf{f}_{\theta,\phi}(\mathbf{z}\_\tau;\mathbf{c},\tau)\rVert\big]
$$
