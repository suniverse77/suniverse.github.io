...
1. 첫 번째 프레임의 latent code를 샘플링한다.
   ...
   $$
2. 첫 번째 프레임에 대해 DDIM backward를 적용해 $x_T^1\rightarrow x_{T'}^1$
5. $k=2,3,\dots,m$에 대해 아래의 과정을 반복하면서 
6. direction을 정의한다. $\delta=(\delta_x,\delta_y)\in\mathbb{R}^2$
7. $\delta_k=\lambda\cdot(k-1)\lambda$
8. $\tilde{x}_{T'}^k$
9. 
